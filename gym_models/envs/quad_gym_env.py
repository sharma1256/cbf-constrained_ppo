...
class QuadDynamics(gym.Env):
    ...
    def __init__(self,
                 ...
                 initial_state={"x": np.array([3.5, 4.9, 0]),
                                "xdot": np.zeros(3,),
                                # ! hardcoded
    ...
    def reset(self):
        ...
        self.state = {"x": np.array([-1.5, -1.5, 0]), #x=4.8 and y=3.5 is a critical point till which the projection based controller works
                       "xdot": np.array([0, 0, 0]),
                       # ! hardcoded
                       "theta": np.radians(np.array([0, 0, 0])),
                       # ! hardcoded
