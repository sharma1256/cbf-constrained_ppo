...
class BetaPolicyBase(PolicyNetwork):
    ...
    def sample(self, obs, action_dim, no_log_prob=False):
        ...
        dist_orig = td.Beta(alphas, betas)
        action_orig = dist_orig.rsample()
        log_prob_orig = dist_orig.log_prob(action_orig)
        #from original implementation
        
        action_along_dims = [dist.rsample() for dist in dists]
        action = torch.tensor(action_along_dims, requires_grad=True)
        #action = torch.tensor(action_along_dims)
        ...
        #below line commented out
        #return action if no_log_prob else (action, log_prob)
        
        
        #new lines added - 08.29.2023
        action_orig = lb + (ub - lb) * action_orig
        return action_orig if no_log_prob else (action_orig, log_prob_orig)

    def log_probs(self, obs, actions, action_dim):
        alphas_arr, betas_arr = self.forward(obs)
        #adding one line
        dists = td.Beta(alphas_arr, betas_arr)
        
        #commenting the below lines 08.26.2023
        dists_new = []
        ...
        #commenting the below lines 08.26.2023
        log_probs = []
        for action, action_dists in zip(actions, dists_new):
            # log_probs.append(
        ...
        #return log_probs #commenting 08/25/23
        return dists.log_prob(actions).flatten()
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                #08/08/2023 -- Changed the entropy coefficient to 0.01--vipul
